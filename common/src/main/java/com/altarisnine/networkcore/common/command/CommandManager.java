...
public abstract class CommandManager {
    ...
    public CommandManager(NetworkCore instance) {
    ...
    public final void registerCommand(CommandNode command, NetworkCore plugin) {
        register(command);
        registeredCommands.put(command.getName(), command);
    ...
    public final List<String> getCompletions(String[] input, boolean console) {
        ...
        /*for (String s : list) {
            Core.getApi().getLogger().debug(s.replace(" ", "*"));
        }*/
        ...
        if (!iterator.hasNext()) {
            ...
            if (iterator.hasNext()) {
                ...
                if (resolved == null) {
                    //Core.getApi().getLogger().debug("No matches for command with next argument provided");
                ...
                if (registeredAliases.keySet().stream().anyMatch(s -> s.equals(cmdToTest))) {
                    //Core.getApi().getLogger().debug("Alias matched, next tab complete will tackle next argument if user adds space");
                    return Collections.emptyList();
                } else {
                    //Core.getApi().getLogger().debug("Return matching aliases (if any)");
        ...
        if (iterator.hasNext()) {
            ...
            do {
                ...
                for (CommandNode child : node.getChildren().values()) {
                    if (child.getAliases().contains(inputArg)) {
                        //Core.getApi().getLogger().debug("Matched child, update node cursor, and continue subcommands loop.");
                        node = child;
            ...
            //Core.getApi().getLogger().debug("No more child matches, start matching arguments");
        ...
        if (iterator.hasNext()) {
            //Core.getApi().getLogger().debug("Found another input arg for argument checking");

            // Get input argument
            String inputArg = iterator.next();

            if (iterator.previousIndex() == startIndex && !iterator.hasNext()) {
                //Core.getApi().getLogger().debug("First non completed child node arg: " + inputArg);
                ...
                if (node.hasArgumentAt(index)) {
                    //Core.getApi().getLogger().debug("Found arguments for child, added to completions");
                ...
                while (iterator.hasNext()) {
                    ...
                    if (node.hasArgumentAt(index)) {
                        ...
                        //Core.getApi().getLogger().debug("Found argument #" + Integer.toString(index) + " with type " + argument.getClass().getSimpleName());

                        if (argument.validate(inputArg)) {
                            //Core.getApi().getLogger().debug(String.format("Validated argument #%d with input %s", index, inputArg));
                        } else {
                            if (!iterator.hasNext()) {
                                ...
                                //Core.getApi().getLogger().debug("No arguments found that match, more arguments found, return no completions");
                                return Collections.emptyList();
                            }
                        }
                    }
