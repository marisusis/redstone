package com.altarisnine.redstone.common.listeners;
...
import com.altarisnine.redstone.api.event.entity.EntityDamageEvent;
import com.altarisnine.redstone.api.event.inventory.InventoryCloseEvent;
import com.altarisnine.redstone.api.event.inventory.interact.InventoryClickEvent;
import com.altarisnine.redstone.api.event.player.PlayerInteractEvent;
import com.altarisnine.redstone.api.event.player.PlayerJoinEvent;
import com.altarisnine.redstone.api.event.player.PlayerMoveEvent;
...
public class CoreListener {
    ...
    public CoreListener(RedstoneCore instance) {
        this.core = instance;
    }
    ...
    public final void onPlayerJoin(User player) {
        core.getNetworkScheduler().runAsync(() -> {
            ...
            core.getEventManager().callEvent(new PlayerJoinEvent(Redstone.getApi().getPlayer(uuid)));
    ...
    public Event onPlayerMove(final Player who, final Location to) {
        ...
        Session session = Redstone.getApi().getSession(who.getUniqueId());
    ...
    protected boolean onPlayerChat(User sender, String message) {
        ...
        if (core.getType() != ServerType.PROXY) {
            ...
            core.getOnlineUsers().forEach(user -> Redstone.getApi().getPlayer(user.getUniqueId()).sendMessage(Text.of(player.getDisplayName() + " &7>> &f" + message)));
